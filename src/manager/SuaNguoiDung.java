...
public class SuaNguoiDung {
	...
	protected void createContents(String manguoidung2, String nhom, String tendangnhap, String tennguoidung,
		...
		// Lấy dữ liệu được truyền vào để sửa
		// ------------------------------------------------------------------------------------------------------------------------
					...
					if (!textManguoidung.getText().isEmpty() && !textTendangnhap.getText().isEmpty()
						...
						if (ketqua > 0) {
							...
							if (ngonngu == 0) {
								...
								thongbao.setMessage("Cập nhật thông tin người dùng thành công!");
							} else if (ngonngu == 1) {
								thongbao.setText("Notification");
								thongbao.setMessage("Update user information successfully!");
							} else if (ngonngu == 2) {
								thongbao.setText("通知");
								thongbao.setMessage("成功更新用戶信息!");
							}
							thongbao.open();
						} else {
							MessageBox thongbao = new MessageBox(shell, SWT.ICON_INFORMATION | SWT.OK);
							if (ngonngu == 0) {
								thongbao.setText("Thông báo");
								thongbao.setMessage("Cập nhật thông tin người dùng không thành công!");
							} else if (ngonngu == 1) {
								thongbao.setText("Notification");
								thongbao.setMessage("Updating user information failed!");
							} else if (ngonngu == 2) {
								thongbao.setText("通知");
								thongbao.setMessage("更新用戶信息失敗!");
							}
							thongbao.open();
						}
					}
